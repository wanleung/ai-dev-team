...
## Project Creation / Scaffold Rules
- For a new Flutter app, create the Flutter project in `mobile/` unless the user or repo instructions name another app directory.
- Use the Flutter tool scaffold as the baseline: `flutter create mobile` from the repo root, or generate the equivalent full Flutter project structure if command execution is unavailable.
- If `flutter` is not on PATH, source the project environment first or use the configured binary: `~/develop/flutter/bin/flutter`.
- Do not submit a Flutter app that only contains `pubspec.yaml` and `lib/`. A valid new Flutter app must include the generated platform/test boilerplate needed by `flutter analyze`, `flutter test`, and builds.
- Preserve generated scaffold directories unless explicitly out of scope: `android/`, `ios/`, `web/`, `test/`, plus platform folders such as `linux/`, `macos/`, and `windows/` when generated by the local Flutter template.
- Put application logic under `mobile/lib/`; do not create a sibling `flutter/` app directory when the project convention says `mobile/`.
- Keep generated files such as `.metadata`, `analysis_options.yaml`, `pubspec.yaml`, and lock files when produced by the Flutter tool.
- After creating or modifying the scaffold, run from repo root: `cd mobile && flutter pub get && flutter analyze && flutter test` when tests exist; replace `flutter` with `~/develop/flutter/bin/flutter` when PATH is not configured.
...
- Flag new BLoC, Provider, GetX, or other state managers unless explicitly requested or already established in the target repo; default new work to Riverpod
